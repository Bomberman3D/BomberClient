...
void GameSettingsStage::OnDraw(uint32 diff)
{
    ...
    if (m_subStage == 0)
    {
       sDisplay->PrintText(MAIN_FONT, 100,100,FONT_SIZE_N, "Pøipojování...");
    ...
    {
       sDisplay->PrintText(MAIN_FONT, 100,100,FONT_SIZE_N, "Inicializace komunikace...");
    }
    else if (m_subStage == 2)
    {
       sDisplay->PrintText(MAIN_FONT, 100,100,FONT_SIZE_N, "Ovìøování...");
    }
    else if (m_subStage == 3)
    {
       sDisplay->PrintText(MAIN_FONT, 100,100,FONT_SIZE_N, "Získávání instance listu...");
    }
    else if (m_subStage == 4)
    {
        ...
        sDisplay->PrintText(MAIN_FONT, 35, 65,FONT_SIZE_N, "%s%s", m_nickfield.c_str(), m_nickselected?" |":"");
        // Mistnosti nadpis
        ...
        {
            sDisplay->PrintText(MAIN_FONT, 5*WIDTHPCT+10, 194.0f+i*27,FONT_SIZE_N,"%s",exploded[(i*4)+0].c_str());
            sDisplay->PrintText(MAIN_FONT, 10*WIDTHPCT  , 194.0f+i*27,FONT_SIZE_N,"%s",exploded[(i*4)+1].c_str());
            sDisplay->PrintText(MAIN_FONT, 50*WIDTHPCT  , 194.0f+i*27,FONT_SIZE_N,"%s",exploded[(i*4)+2].c_str());
            sDisplay->PrintText(MAIN_FONT, 70*WIDTHPCT  , 194.0f+i*27,FONT_SIZE_N,"%s",exploded[(i*4)+3].c_str());
        }
    ...
    else if (m_subStage == 100)
    {
        ...
        switch (m_selectedMode)
        {
            case GAME_TYPE_SP_CLASSIC:
                sDisplay->PrintText(FONT_ONE, 140, 45,FONT_SIZE_3, "Rychlá hra");

                // TODO: funkce pro odstavcovy text a zalamovani
                sDisplay->Draw2D(15, 80, 130, WIDTH-80-80, HEIGHT-130-130);
                sDisplay->PrintText(FONT_ONE, 100, 150,FONT_SIZE_4, "Rychlá hra je základním typem hry. Princip je inspirován pùvodní pøedlohou.");
                sDisplay->PrintText(FONT_ONE, 100, 170,FONT_SIZE_4, "Obsahuje klasické krabice, bonusy, nepøátele. Podmínkou výhry je zdolat");
                sDisplay->PrintText(FONT_ONE, 100, 190,FONT_SIZE_4, "všechny nepøátele v èasovém limitu.");
                break;
            case GAME_TYPE_SP_MEME:
                sDisplay->PrintText(FONT_ONE, 140, 45,FONT_SIZE_3, "Meme wars");

                // TODO: funkce pro odstavcovy text a zalamovani
                sDisplay->Draw2D(15, 80, 130, WIDTH-80-80, HEIGHT-130-130);
                sDisplay->PrintText(FONT_ONE, 100, 150,FONT_SIZE_4, "Meme wars je typ hry inspirovaný internetovými meme postavièkami a");
                sDisplay->PrintText(FONT_ONE, 100, 170,FONT_SIZE_4, "srandièkami, jako napøíklad troll, nyan cat, rickroll, rage comics, pedobear,");
                sDisplay->PrintText(FONT_ONE, 100, 190,FONT_SIZE_4, "o rly?, a další. Úkolem je opìt znièit všechny nepøátele v èasovém limitu,");
                sDisplay->PrintText(FONT_ONE, 100, 210,FONT_SIZE_4, "ovšem za pomoci bonusù, které se v originální høe neobjevily. Co tøeba Nyan");
                sDisplay->PrintText(FONT_ONE, 100, 230,FONT_SIZE_4, "cat speed bonus?");

                sDisplay->PrintText(FONT_ONE, 100, 270,FONT_SIZE_4, "NYI");
                break;
            default:
                break;
        }
    ...
    else if (m_subStage == 101)
    {
        ...
        switch (m_selectedMode)
        {
            case GAME_TYPE_SP_CLASSIC:
            {
                sDisplay->PrintText(FONT_ONE, WIDTH/2-64*4+4*36/2, 50,FONT_SIZE_N, "Rychlá hra");

                sDisplay->PrintText(FONT_ONE, 50, 150,FONT_SIZE_4, "Poèet nepøátel");
                sDisplay->PrintText(MAIN_FONT, 105, 183,FONT_SIZE_N, "%u", sGameplayMgr->GetSetting(SETTING_ENEMY_COUNT));
                sDisplay->Draw2D(43, 80, 180, 18, 20);
                sDisplay->Draw2D(41, 140, 180, 18, 20);

                uint32 spacing = 0;
                uint32 leftspacing = 300;
                sDisplay->PrintText(FONT_ONE, 50+leftspacing, 150+spacing,FONT_SIZE_4, "Intenzita beden");
                sDisplay->PrintText(MAIN_FONT, 105+leftspacing, 183+spacing,FONT_SIZE_N, "%u", sGameplayMgr->GetSetting(SETTING_BOX_DENSITY));
                sDisplay->Draw2D(43, 80+leftspacing, 180+spacing, 18, 20);
                sDisplay->Draw2D(41, 140+leftspacing, 180+spacing, 18, 20);

                spacing = 80;
                sDisplay->PrintText(FONT_ONE, 50, 150+spacing,FONT_SIZE_4, "Inteligence nepøátel");
                sDisplay->PrintText(MAIN_FONT, 105, 183+spacing,FONT_SIZE_N, "%u", sGameplayMgr->GetSetting(SETTING_ENEMY_AI_LEVEL));
                sDisplay->Draw2D(43, 80, 180+spacing, 18, 20);
                sDisplay->Draw2D(41, 140, 180+spacing, 18, 20);

                sDisplay->PrintText(FONT_ONE, 50+leftspacing, 150+spacing,FONT_SIZE_3, "Mapa");
                ...
                sDisplay->PrintText(MAIN_FONT, 105+leftspacing, 183+spacing,FONT_SIZE_N, "%s", mapname.c_str());
                sDisplay->Draw2D(43, 80+leftspacing, 180+spacing, 18, 20);
                sDisplay->Draw2D(41, 380+leftspacing, 180+spacing, 18, 20);

                spacing = 2*80;
                sDisplay->PrintText(FONT_ONE, 50, 150+spacing,FONT_SIZE_4, "Rychlost nepøátel");
                sDisplay->PrintText(MAIN_FONT, 105, 183+spacing,FONT_SIZE_N, "%u", sGameplayMgr->GetSetting(SETTING_ENEMY_SPEED));
                sDisplay->Draw2D(43, 80, 180+spacing, 18, 20);
                sDisplay->Draw2D(41, 140, 180+spacing, 18, 20);
                break;
            }
            case GAME_TYPE_SP_MEME:
            {
                sDisplay->PrintText(FONT_ONE, WIDTH/2-64*4+4*36/2, 50,FONT_SIZE_N, "Meme wars");
                break;
            }
        }
